...
export class Template {
  ...
  private generateAnnotations(params: Params): Record<string, string> | undefined {
    ...
    return {
      ...annotations,
      ...this.generateDatadogAgent(params.applicationName),
    };
  }
  ...
  private createSidecarNginxContainer(params: SidecarParams): k8s.PodTemplate.Container {
    return {
      name: params.applicationName,
      env: [],
      image: `nginx:stable`,
      ports: [
        {
          containerPort: Port.ClusterPort.Sidecar,
        },
      ],
      livenessProbe: {
        failureThreshold: 3,
      ...
      resources: params.resources || this.defatulMinimumResource,
      volumeMounts:
        params.volumeMounts[params.applicationName] && params.volumeMounts[params.applicationName].length > 0
          ? params.volumeMounts[params.applicationName]
          : undefined,
  ...
  private createContainers(params: Params): k8s.PodTemplate.Container[] {
    if (params.sidecar) {
      return [this.createSidecarNginxContainer(params.sidecar), this.createAppContainer(params)];
    }
  ...
  /**
   * @see https://docs.datadoghq.com/ja/agent/kubernetes/log/?tab=daemonset#%E3%82%B3%E3%83%B3%E3%83%95%E3%82%A3%E3%82%AE%E3%83%A5%E3%83%AC%E3%83%BC%E3%82%B7%E3%83%A7%E3%83%B3
   */
  private generateDatadogAgent(containerName: string) {
    const data = [
      {
        source: "nodejs",
        service: `your-prefix-${containerName}`,
    ...
    return {
      [`ad.datadoghq.com/${containerName}.logs`]: JSON.stringify(data, null, 2),
    };
  }
}
